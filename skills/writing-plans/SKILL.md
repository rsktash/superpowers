...
A plan is never written over an unresolved design fork; rival architectures, an unvalidated dependency, a prototype-only shape stop decomposition. Triage before parking: a line colliding with a ruling, code, or spec is a defect to rewrite; repo-answered questions and one-safe-answer defaults are pruned; unverified claims researched, not asked. Export `BD_ACTOR=planner` before your first bd write, all run: bd bylines your questions and findings planner, refusing your rulings. Reuse a slug `bd topics` prints; mint only when none fits. One command per survivor: `bd create "Decide: <fork>" --parent <root> --question "<fork>" --topic <slug>`, held out of `bd ready` by its question; `bd dep add <task> <decide-id>` for every task it blocks. A fork on an already-created task: `bd question add <task-id> "<fork>" --topic <slug>`. The coordinator or owner rules each separately: `bd ruling add <decide-id> "<text>" --answers <Q-id> --close`. Forks outnumbering tasks → superpowers-beads:wayfinder.
