...
4. **`bd dep add <task> <decide-id>` for every task the fork blocks** (here Tasks 5 and 6). An un-dep-linked Decide bead is an orphan fork: the execution skills' epic gate reads `rdeps` and stops on it.
