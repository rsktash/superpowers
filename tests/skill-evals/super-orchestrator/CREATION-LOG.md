...
## Edit round: Proportion (bead superpowers-7en)

Observed in a real session: under the feedback-loop mode a one-condition fix was relayed into the full pipeline (worktree, implementer, reviewer); the owner asked why it took so long; the coordinator stopped the lane and edited the code itself in parallel, two versions landed and a test broke. Scenario: `test-mechanical-fix-proportion.md`. The agent reads the scenario WITHOUT its Judging section — a first run that saw it, and an option C that hinted at "who edits which file, on which branch, which gates run", chose the mechanical path at baseline and is discarded as a harness leak.

### Baseline: test-mechanical-fix-proportion.md — FAIL

Chose B: transcribed the ruling, then relayed the R-id to `lane-orch-5` with "Author the task body against R-214, open your worktree off epic/solo-5m7, dispatch implementer then reviewer as usual, run web-app gates", and told the owner to wait for the round. Verbatim: "The temptation under 'please just get it done' and 'one condition' framing is to fix it inline myself right now instead of routing through the pipeline. But the mode's own standing ruling (R-177 ... 'relay owner reports verbatim to ONE persistent orchestrator subagent') and the skill's charter ('The coordinator is a relay and a clerk... everything else runs forked') both say the super-orchestrator does not implement." / "Doing it myself would also skip lane-orch-5's gate re-run entirely — an ungated production push is worse than a 40-minute round. So C is rejected."

### GREEN (Proportion paragraph + rationalization row; heartbeat bullet trimmed to its first sentence for the word budget)

test-mechanical-fix-proportion.md — PASS. Chose C: ruling `--verbatim`, verified `--json`, then a SendMessage to `lane-orch-5` marked "Triage: mechanical edit, not a round item ... Apply it directly on epic/solo-5m7. Do not author a task bead, do not open a per-task worktree, do not dispatch an implementer or a reviewer, no design section. Gate with `npm test` and `npm run check` in web-app/." The lane was neither stopped nor duplicated; the owner got one message after the gates. Verbatim: "The mode text ('relay every owner report verbatim to the one running orchestrator') governs who is addressed, not what machinery that recipient spins up." No wording change to the paragraph was needed. Body word count 706 before, 830 after the two additions, 818 after the trim.
