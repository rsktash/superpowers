...
one; when none fits, file with `--topic pending-<task-id>` for now, a slug
the coordinator resolves at Close, until bd stores pending topic statements
itself, at which point pending is bd's own typed state in place of that slug
and an unresolved one still gets a slug from the coordinator at Close — a
finding never ends with no topic.
