...
```
cd tests/claude-code && for t in test-completion-gate test-gate-lint test-fail-streak-guard test-review-package test-structural-index test-structural-index-kotlin test-structural-index-swift test-structural-index-python test-map-check; do ./$t.sh; done
```
