...
Where a `symbol` span starts, per backend: TypeScript starts at the declaration's own first token, a leading `export` keyword included (`class UndoHistory` at `tests/fixtures/structural-index/ts/src/history.ts:1-5` starts on the `export class` line). Go starts at the `func`/`type`/`var`/`const` keyword's own line (`func ValidateStatus` at `tests/fixtures/structural-index/go/status.go:3-5`). Kotlin starts at the declaration node's own first line — the `class`/`fun`/`const val` keyword (`class BadgeCard` at `tests/fixtures/structural-index/kotlin/src/Badge.kt:13-23`; `const val BADGE_LIMIT` at `Badge.kt:3-3`). Swift starts at the declaration node's own first line, a leading attribute included (`struct AttrBadge`, preceded by `@MainActor`, at `tests/fixtures/structural-index/swift/src/Badge.swift:25-28`). Python starts at the `def`/`class` keyword's own line (`class BadgeCard` at `tests/fixtures/structural-index/python/src/badge.py:8-13`; `def renderBadge` at `badge.py:4-5`).

`languages` prints one line per language the target repository tracks: `<id>`, its tracked file count, and its backend, tab-separated, ordered typescript, go, kotlin, swift, python, other — backend `compiler` for typescript, go, kotlin, swift, and python, `none` for other. A name whose definitions live only in `none` languages still gets `callers` and `tests` answers, produced by word-boundary text search and marked by one stderr line, quoted exactly: `structural-index: <query> <name> answered by text search over <n> files in <ids>`. A `symbol` query in the same position prints `symbol not found: <name>` followed by `structural-index: no definition backend for <ids>; not searched` — a text answer's grade, never a missing symbol.

Each language is generated only when the target repository tracks files of that language: `<root>/.bd/index/typescript.json` for `*.ts`/`*.tsx`, `<root>/.bd/index/go.json` for `*.go`, `<root>/.bd/index/kotlin.json` for `*.kt`/`*.kts`, `<root>/.bd/index/swift.json` for `*.swift`, `<root>/.bd/index/python.json` for `*.py`, plus `<root>/.bd/index/metadata.json`. The metadata records the target repository's HEAD and the indexed languages. An absent index or changed HEAD regenerates and reports per-language wall time on stderr; reuse on the same HEAD is silent, and `--regen` forces regeneration. The TypeScript package is resolved from a tracked package directory inside the target repository, then from `STRUCTURAL_INDEX_TYPESCRIPT_PACKAGE`; when neither holds one, the CLI exits 2 and names the variable. The Go backend runs Go's own go/parser + go/types through the `go` toolchain on PATH. The Kotlin backend runs tree-sitter (0.21.1) with the tree-sitter-kotlin (0.3.8) grammar, resolved the same way: `node_modules/tree-sitter` and `node_modules/tree-sitter-kotlin` under a tracked package directory inside the target repository first, then the `STRUCTURAL_INDEX_KOTLIN_TOOLCHAIN` directory; when neither holds them, the CLI exits 2 and names the variable. The Swift backend runs tree-sitter (0.22.1) with the tree-sitter-swift (0.7.1) grammar, resolved the same way: `node_modules/tree-sitter` and `node_modules/tree-sitter-swift` under a tracked package directory inside the target repository first, then the `STRUCTURAL_INDEX_SWIFT_TOOLCHAIN` directory; when neither holds them, the CLI exits 2 and names the variable. The Python backend runs the standard-library ast + symtable parser (the interpreter is the toolchain, per R-35 and R-31): the `python3` interpreter is resolved from the target repository's own virtual environment first (`.venv/bin/python3` or `venv/bin/python3` beside a tracked `pyproject.toml` or `requirements*.txt`), then from the `STRUCTURAL_INDEX_PYTHON_TOOLCHAIN` directory holding `bin/python3`; when neither holds one, the CLI exits 2 and names the variable.

## Roster runbook

On first dispatch against a target repository, run the roster query, record the line it prints per language, and export the toolchain variable each `compiler`-backed language needs before the first `symbol`/`callers`/`tests` query. Copy and fill in:

```
scripts/structural-index languages --repo <root>
# <id>	<tracked-file-count>	<backend>
# (one line per language the repository tracks; `other` closes the roster)

export STRUCTURAL_INDEX_TYPESCRIPT_PACKAGE=<dir holding lib/typescript.js>
export STRUCTURAL_INDEX_KOTLIN_TOOLCHAIN=<dir holding node_modules/tree-sitter + node_modules/tree-sitter-kotlin>
export STRUCTURAL_INDEX_SWIFT_TOOLCHAIN=<dir holding node_modules/tree-sitter + node_modules/tree-sitter-swift>
export STRUCTURAL_INDEX_PYTHON_TOOLCHAIN=<dir holding bin/python3>
```

Export only the variable a roster row with backend `compiler` needs, and skip any language a tracked in-repo directory already resolves (a package directory for TypeScript, Kotlin, or Swift; a `.venv`/`venv` beside a tracked `pyproject.toml` or `requirements*.txt` for Python) — Go needs no variable, only the `go` toolchain on `PATH`. A row the roster does not list needs no variable at all.
